// class HINKE042A11 : Display class for HINKE042A11
// Author : J-M Zingg
// Edited : ATCnetz.de
//
...
#define HINKE042A11_PU_DELAY 500

HINKE042A11::HINKE042A11(GxIO& io, int8_t rst, int8_t busy)
  : GxEPD(HINKE042A11_WIDTH, HINKE042A11_HEIGHT), IO(io),
    _current_page(-1), _using_partial_mode(false), _diag_enabled(false),
    _rst(rst), _busy(busy)
{
}

void HINKE042A11::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  swap(x, y);
  ...
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = HINKE042A11_WIDTH - x - 1;
      break;
    case 2:
      x = HINKE042A11_WIDTH - x - 1;
      y = HINKE042A11_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = HINKE042A11_HEIGHT - y - 1;
  ...
  uint16_t i = x / 8 + y * HINKE042A11_WIDTH / 8;
  if (_current_page < 1)
  {
  ...
  {
    y -= _current_page * HINKE042A11_PAGE_HEIGHT;
    if ((y < 0) || (y >= HINKE042A11_PAGE_HEIGHT)) return;
    i = x / 8 + y * HINKE042A11_WIDTH / 8;
  }
...
void HINKE042A11::init(uint32_t serial_diag_bitrate)
{
...
void HINKE042A11::fillScreen(uint16_t color)
{
  uint8_t black = 0x00;
  uint8_t red = 0xFF;
...
void HINKE042A11::update(void)
{
 // reset required for wakeup
  ...
  for (uint32_t i = 0; i < HINKE042A11_BUFFER_SIZE; i++)
  {
...
void HINKE042A11::_writeCommand(uint8_t command)
{
  IO.writeCommandTransaction(command);
}

void HINKE042A11::_writeData(uint8_t data)
{
  IO.writeDataTransaction(data);
}

void HINKE042A11::_waitWhileBusy(const char* comment)
{
...
void HINKE042A11::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      swap(w, h);
      x = HINKE042A11_WIDTH - x - w - 1;
      break;
    case 2:
      x = HINKE042A11_WIDTH - x - w - 1;
      y = HINKE042A11_HEIGHT - y - h - 1;
      break;
    case 3:
      swap(x, y);
      swap(w, h);
