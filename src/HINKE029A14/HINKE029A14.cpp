// class HINKE029A14 : Display class for HINKE029A14 
// Author : J-M Zingg
// Edited : ATCnetz.de
//
...
#define HINKE029A14_PU_DELAY 500

HINKE029A14::HINKE029A14(GxIO& io, int8_t rst, int8_t busy)
  : GxEPD(HINKE029A14_WIDTH, HINKE029A14_HEIGHT), IO(io),
    _current_page(-1), _using_partial_mode(false), _diag_enabled(false),
    _rst(rst), _busy(busy)
{
}

void HINKE029A14::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  swap(x, y);
  ...
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = HINKE029A14_WIDTH - x - 1;
      break;
    case 2:
      x = HINKE029A14_WIDTH - x - 1;
      y = HINKE029A14_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = HINKE029A14_HEIGHT - y - 1;
  ...
  uint16_t i = x / 8 + y * HINKE029A14_WIDTH / 8;
  if (_current_page < 1)
  {
  ...
  {
    y -= _current_page * HINKE029A14_PAGE_HEIGHT;
    if ((y < 0) || (y >= HINKE029A14_PAGE_HEIGHT)) return;
    i = x / 8 + y * HINKE029A14_WIDTH / 8;
  }
...
void HINKE029A14::init(uint32_t serial_diag_bitrate)
{
...
void HINKE029A14::fillScreen(uint16_t color)
{
  uint8_t black = 0x00;
  uint8_t red = 0xFF;
...
//2.9" Display:
  ...
  for (uint32_t i = 0; i < HINKE029A14_BUFFER_SIZE; i++)
  {
...
void HINKE029A14::_writeCommand(uint8_t command)
{
  IO.writeCommandTransaction(command);
}

void HINKE029A14::_writeData(uint8_t data)
{
  IO.writeDataTransaction(data);
}

void HINKE029A14::_waitWhileBusy(const char* comment)
{
...
void HINKE029A14::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      swap(w, h);
      x = HINKE029A14_WIDTH - x - w - 1;
      break;
    case 2:
      x = HINKE029A14_WIDTH - x - w - 1;
      y = HINKE029A14_HEIGHT - y - h - 1;
      break;
    case 3:
      swap(x, y);
      swap(w, h);
