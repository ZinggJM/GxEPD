/************************************************************************************
   class GxGDEW029Z10 : Display class example for GDEW029Z10 e-Paper from Dalian Good Display Co., Ltd.: www.good-display.com

   based on Demo Example from Good Display, available here: http://www.good-display.com/download_detail/downloadsId=522.html
...
GxGDEW029Z10::GxGDEW029Z10(GxIO& io, uint8_t rst, uint8_t busy)
...
void GxGDEW029Z10::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation)
{
  ...
  uint16_t xe = min(GxGDEW029Z10_WIDTH, x + w) - 1;
  uint16_t ye = min(GxGDEW029Z10_HEIGHT, y + h) - 1;
...
void GxGDEW029Z10::_writeToWindow(uint16_t xs, uint16_t ys, uint16_t xd, uint16_t yd, uint16_t w, uint16_t h, bool using_rotation)
{
  ...
  uint16_t xde = min(GxGDEW029Z10_WIDTH, xd + w) - 1;
  uint16_t yde = min(GxGDEW029Z10_HEIGHT, yd + h) - 1;
...
void GxGDEW029Z10::drawPagedToWindow(void (*drawCallback)(void), uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ...
  for (_current_page = 0; _current_page < GxGDEW029Z10_PAGES; _current_page++)
  {
    uint16_t yds = max(y, _current_page * GxGDEW029Z10_PAGE_HEIGHT);
    uint16_t yde = min(y + h, (_current_page + 1) * GxGDEW029Z10_PAGE_HEIGHT);
    if (yde > yds)
    {
      fillScreen(GxEPD_WHITE);
      drawCallback();
