// class GxGDEW0213I5F : Display class for GDEW0154Z17 e-Paper from Dalian Good Display Co., Ltd.: www.good-display.com
//
...
#define GxGDEW0213I5F_PU_DELAY 500
...
void GxGDEW0213I5F::drawExamplePicture(const uint8_t* black_bitmap, const uint8_t* red_bitmap, uint32_t black_size, uint32_t red_size)
{
  drawPicture(black_bitmap, red_bitmap, black_size, red_size, bm_invert_red);
}

void GxGDEW0213I5F::drawPicture(const uint8_t* black_bitmap, const uint8_t* red_bitmap, uint32_t black_size, uint32_t red_size, int16_t mode)
{
  if (_current_page != -1) return;
  _using_partial_mode = false;
  _wakeUp();
...
void GxGDEW0213I5F::_writeToWindow(uint16_t xs, uint16_t ys, uint16_t xd, uint16_t yd, uint16_t w, uint16_t h, bool using_rotation)
{
  if (using_rotation)
  {
    switch (getRotation())
...
void GxGDEW0213I5F::_wakeUp()
{
  ...
  _writeData (GxGDEW0213I5F_WIDTH); // HRES=104
  _writeData (GxGDEW0213I5F_HEIGHT >> 8); // VRES_byte1=0
  _writeData (GxGDEW0213I5F_HEIGHT & 0xFF); // VRES_byte2=212
...
void GxGDEW0213I5F::drawPagedToWindow(void (*drawCallback)(void), uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ...
  for (_current_page = 0; _current_page < GxGDEW0213I5F_PAGES; _current_page++)
  {
    ...
    uint16_t yde = gx_uint16_min(y + h, (_current_page + 1) * GxGDEW0213I5F_PAGE_HEIGHT);
    if (yde > yds)
    {
      fillScreen(GxEPD_WHITE);
      drawCallback();
